...
void Game::SetupResources(void){
	...
	std::string filename = std::string(MATERIAL_DIRECTORY) + "/shaders" + std::string("/textured_material");
	resman_.LoadResource(Material, "TextureShader", filename.c_str());
	
	// shader for corona effect
	filename = std::string(MATERIAL_DIRECTORY) + "/shaders" + std::string("/corona");
	resman_.LoadResource(Material, "Procedural", filename.c_str());


	// shader for 3-term lighting effect
	filename = std::string(MATERIAL_DIRECTORY) + "/shaders" + std::string("/lit");
	resman_.LoadResource(Material, "Lighting", filename.c_str());

    // Load texture to be used on the object
	filename = std::string(MATERIAL_DIRECTORY) + "/textures" + std::string("/rocky.png");

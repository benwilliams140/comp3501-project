...
namespace game {
	...
	void HoverTank::motionControl() {
		...
		if (velocity_.x != 0.0f) {
			if (velocity_.x > 0.0f)      velocity_.x -= friction;
			else if (velocity_.x < 0.0f) velocity_.x += friction;
		}
		if (velocity_.z != 0.0f) {
			if (velocity_.z > 0.0f)      velocity_.z -= friction;
			else if (velocity_.z < 0.0f) velocity_.z += friction;
		}
